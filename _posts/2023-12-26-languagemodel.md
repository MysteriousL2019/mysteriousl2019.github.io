...
### 密度估计建模
* 密度估计的目标是构建一个模型 $ p_{\theta} $, 他能准确的表示数据的整体分布，可以做到回答任意的概率推断查询，这样就可以计算出数据集中任何事件的概率，包括条件概率和边缘概率。
* 在此情况下，可以将学习问题视作密度估计问题，目标是构建一个分布 $p_{\theta}$，使其尽可能接近真实数据 $p_{data}$
### 评估“接近程度的方法”
* 为了评估构建的分布 $p_{\theta}$, 和真实数据 $p_{data}$之间的差异。通常有以下几种方法
* [KL散度](https://www.bilibili.com/video/BV1CS411N72a/?spm_id_from=333.788&vd_source=3ec36be4e958d9ea45af4cd9fda18389)：度量了两个概率分布之间的差异，我们希望最小化 $p_{\theta}$相对于 $p_{data}$ 的KL散度。可以理解为对于分布的交叉墒的优化。
    * 假设有两个概率分布P和Q，它们分别描述了一组事件的概率。例如，P是真实分布，Q是模型估计的分布。KL散度衡量的是，如果用Q来近似P，会有多大的信息损失。
    * 信息损失：KL散度可以看作是额外的信息损失。它告诉我们，如果实际分布是P，但我们用Q来编码信息，会有多少额外的“信息”需要传输。这种额外的信息量可以理解为编码不准确带来的“代价”。它量化了使用Q来近似P的误差。
* 似然函数（likehood）：最大化训练数据在模型下的似然，即 $P_{\theta}(D)$。 这意味着希望模型能够很好的解释已知数据。
* 交叉墒：交叉墒度量了两个分布之间的差异。我们希望最小化模型分布 $p_{\theta}$ 和真实数据之间的交叉墒，等价于最大化对数似然函数。

### 大模型幻觉解决
* 1.更好的训练数据：提供更丰富、更高质量的训练数据，以便模型能够学习到更准确的知识和更合适的生成策略。这包括使用更大的训练语料库、去除训练数据中的错误和偏见、以及添加更多领域特定的数据。
* 2.生成控制策略：使用生成控制策略，如 Top-K 采样、Top-P 采样（也称为核采样）或者温度调整，来限制模型的输出。这些策略可以降低生成文本时的随机性，从而减少模型幻觉的出现。
    * Top-K 采样：在生成下一个词时，模型会计算词汇表中每个词的概率，然后选择概率最高的 K 个词，随机选择一个作为输出。这样可以防止模型选择到概率极低的词，从而提高生成文本的质量。但是，如果 K 设置得过小，可能会导致生成的文本过于单一和重复。
    * Top-P 采样（也称为核采样）：与 Top-K 采样类似，但是选择的是累积概率超过 P 的最少的词。例如，如果 P=0.9，那么就选择累积概率超过 0.9 的最少的词。这样可以保证生成的随机性，同时避免选择到概率极低的词。
    * 温度调整：温度是一个控制模型输出概率分布的参数。温度越高，生成的文本越随机和多样；温度越低，生成的文本越确定和一致。具体来说，温度是用来调整模型输出的 softmax 函数的。在 softmax 函数中，每个词的概率是通过其对应的 logits（模型输出的原始分数）的指数除以所有 logits 的指数之和来计算的。通过温度调整，可以改变 logits 的大小，从而改变 softmax 函数的输出概率分布。
        * softmax 函数用于将模型的原始输出（也称为 logits）转换为概率分布。在这个过程中，每个 logit 首先会被指数化（即计算 e 的 logit 次方），然后除以所有 logits 的指数之和。这样可以确保所有的输出概率都在 0 到 1 之间，并且所有概率之和为 1。
        * 这里的"温度"就是用来调整 logits 的大小。在计算指数之前，每个 logit 都会被除以温度。如果温度大于 1，那么 logits 会变小，这会使得概率分布更均匀，生成的文本更随机和多样；如果温度小于 1，那么 logits 会变大，这会使得概率分布更尖锐，生成的文本更确定和一致。
        * 例如，假设模型的原始输出是 [1, 2, 3]，如果温度为 1（默认值），那么 softmax 的输出是 [0.09, 0.24, 0.67]；如果温度为 2，那么 softmax 的输出是 [0.18, 0.24, 0.58]；如果温度为 0.5，那么 softmax 的输出是 [0.02, 0.12, 0.86]。可以看到，随着温度的增大，输出概率越来越均匀；随着温度的减小，输出概率越来越尖锐。
* 3.约束生成：在生成过程中添加约束，以确保生成的文本满足特定的条件。这可以通过词汇表限制、模板填充或者其他自定义规则来实现。
* 4.生成后的重排（Reranking）：在生成多个候选输出后，使用一个额外的模型（如分类器或排序器）来对这些输出进行评分和重排。这样可以选择最符合预期的输出，从而减少模型幻觉的影响。
* 5.可解释性和可视化：通过可解释性技术和可视化工具，分析模型在生成过程中的注意力机制和隐藏层状态。这有助于了解模型的行为，从而找到可能导致模型幻觉的原因，并采取相应的措施来解决问题。
* 6.反馈和迭代：在实际应用中，收集用户对生成结果的反馈，并将这些反馈用于改进模型。这可以通过在线学习、强化学习或其他增量学习方法来实现。
